...
```sh
python3 -m venv ~/.bip322 && ~/.bip322/bin/pip install "bip322-reports[kernel] @ git+ssh://git@github.com/embeddednation/bip322-reports.git@v0.2.3"
...
bip322-reports [--cli CMD] [-w NAME] report (--year Y | --from WHEN --to WHEN | --from-height H --to-height H)
                                            [--ledger DIR]... [--label NAME] [--history FILE]
...
* The bundles used, with their verification results, and bundles still
  awaiting signatures.
